...
public class StudentController {
	...
	public String studentDetails(@Valid @ModelAttribute Student student, BindingResult result, HttpServletRequest model){
		...
		int year1 = Integer.parseInt(model.getParameter("Year"));
		 ...
		 dbc.getConnection();
		 dbc.createTable();
		 dbc.InsertStudentInfoValues(info);

	        String NameRet="",CourseRet="",CurrentDateRet="";
	        int i=0,FreRet=0,YearRet=0;
	         String Name1 = dbc.RetrieveUserInformationValues().get(dbc.Info.size()-1).getName();
	         String Course1 = dbc.RetrieveUserInformationValues().get(dbc.Info.size()-1).getCourse();
	         int Fee1 = dbc.RetrieveUserInformationValues().get(dbc.Info.size()-1).getFee();
	         int Year1 = dbc.RetrieveUserInformationValues().get(dbc.Info.size()-1).getYear();
	         String CurrentDate1 = dbc.RetrieveUserInformationValues().get(dbc.Info.size()-1).getCurrentdate();
	         dbc.closeConnection();
